...
private[spores] class MacroImpl[C <: Context with Singleton](val c: C) {
      ...
      val traverser = new Traverser {
        override def traverse(tree: Tree) {
          tree match {
            ...
            case th: This =>
              c.error(tree.pos, "invalid reference to [this] " + th.symbol)
  ...
  def checkNullary(funTree: c.Tree, rtpe: c.Type): c.Tree = {
    debug(s"SPORES: enter checkNullary, tree:\n$funTree")
...
//      ) (funBody)
  ...
  def check(funTree: c.Tree, ttpe: c.Type, rtpe: c.Type): c.Tree = {
    ...
    if (paramSym != null) {
      ...
      if (validEnv.isEmpty) {
        // replace reference to paramSym with reference to applyParamSymbolbstituter.transform(funBody.asInstanceOf[symtable.Tree])
...
//          else q"$initializerName.captured = (..$rhss)"
        ...
        val constructorParams = List(List(toTuple(rhss)))
//        if (validEnv.size == 1) q"${rhss.head}"
//          else q"(..$rhss)"
